parfun<-function(args,simfun) {
    ...
    f<-function(y,pctt.tab) {
        ...
        if (simfun=="pcm") {
            ...
            om00<-imv_c(y,p1='p0',p2='graded',pctt.tab) #wrong one
        ...
        if (simfun=="graded") {
            ...
            om00<-imv_c(y,p1='p0',p2='gpcm',pctt.tab) #wrong one
...
############################
    ...
    f<-function(y,pctt.tab) {
        ...
        if (simfun=="pcm") {
            ...
            om00<-imv_t(y,p1='p0',p2='graded',pctt.tab) #wrong one
        ...
        if (simfun=="graded") {
            ...
            om00<-imv_t(y,p1='p0',p2='gpcm',pctt.tab) #wrong one
...
f<-function(tab,index) {
    ...
    names(z)<-c("K","n","J","om1","oracle","overfit","om0","om00")
    z
}
t1a<-f(tab1,1)
t1b<-f(tab1,2)
t2a<-f(tab2,1)
t2b<-f(tab2,2)
tab<-list(t1a=t1a,t2a=t2a,t1b=t1b,t2b=t2b)
...
pf<-function(t1a,t1b) {
    z<-t1a
    plot(NULL,xlim=c(1-3,nrow(z)),ylim=c(0,.4),xlab="",xaxt="n",ylab="IMV(CTT,IRT)")
    ...
    lines(1:nrow(t1a),t1a$om00,type='b',col='blue',pch=19,lwd=1,lty=2)
    text(0,t1a$om0[1],expression(omega[c]),col='blue')
    lines(1:nrow(t1b),t1b$om0,type='b',col='red',lwd=2,pch=19)
    lines(1:nrow(t1b),t1b$om00,type='b',col='red',pch=19,lwd=1,lty=2)
    text(0,t1b$om0[1],expression(omega[t]),col='red')
}
pf(t1a,t1b)
legend("topleft",bty='n',title='DGM=GRM',lty=c(1,2),lwd=c(2,1),c("IRT=GRM","IRT=GPCM"))
pf(t2a,t2b)
legend("topleft",bty='n',title='DGM=GPCM',lty=c(1,2),lwd=c(2,1),c("IRT=GPCM","IRT=GRM"))
dev.off()
...
mtext(side=2,line=2,"IMV(GPCM,GRM)")
pf(t2a,t2b)
legend("topleft",bty='n','DGM=GPCM')
mtext(side=2,line=2,"IMV(GRM,GPCM)")
##
pf<-function(t1a,t1b) {
    z<-t1a
