## ##make probabilities
## getprob<-function(model,th,b,a=1) {
##     pcm<-function(th,b,a) {
##         K<-length(b)+1
##         psi<-list()
...
## simresp<-function(model,th,b,a=1) {
...
##     inits<-c(seq(-1,1,length.out=K-1))
...
## th<-sort(rnorm(N))
## b<-c(b0,b1)
## resp<-simresp('pcm',th,b) #this is just a vector of responses for one item
